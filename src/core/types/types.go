...
func (this CCKind) String() string {
	switch this {
	case Stack:
		return "stack"
	case Reg:
...
func ConvertCC(s string) CCKind {
	switch s {
	case "stack":
		return Stack
	case "reg":
	...
	case "reg_c":
		return Reg_C
	default:
		return InvalidCCKind
	}
}

const (
	InvalidCCKind CCKind = iota
	Stack
...
var T_MainProc = &Type{Proc: &ProcType{CC: Stack, Args: []*Type{}, Rets: []*Type{}}}
